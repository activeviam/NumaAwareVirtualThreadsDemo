...
/**
 * The list of Linux error numbers.
 *
 * @author ActiveViam
 */
public class Errno {
	...
	// CHECKSTYLE.OFF: JavadocVariable (Internal code, with inline doc)
	...
	public static final String[] ERROR_NUMBERS = new String[] {
			// No error has code 0, so last code was valid.
			"NO_ERROR_OCCURRED",
			"EPERM",
			"ENOENT",
			"ESRCH",
	 ...
	 *
	 * @param nativeMethodName The name of the native method called.
	 * @param arguments The arguments with whom the method has been called.
	 */
	public static void throwLastError(final String nativeMethodName, final Object... arguments) {
		final int errno = Native.getLastError();
		throwError(errno, nativeMethodName, arguments);
	}

	/**
	 * Throws a runtime exception for the givven errno.
	 *
	 * @param errno The errno that happened.
	 * @param nativeMethodName The name of the native method call responsible for this errno.
	 * @param arguments The arguments with whom the method was called.
	 */
	public static void throwError(final int errno, final String nativeMethodName, final Object... arguments) {
		throw new RuntimeException(
				"We encountered error number " + errno
						+ (errno < ERROR_NUMBERS.length
								? (" (" + ERROR_NUMBERS[errno] + ")")
								: "(Have a look at /usr/include/asm-generic/errno.h)")
						+ " when performing native call " + nativeMethodName + "(" + join(", ", arguments) + ")");
	}

	@SafeVarargs
	private static <T> CharSequence join(final String separator, final T... args) {
